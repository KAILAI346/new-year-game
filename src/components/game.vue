...
export default {
  ...
  methods: {
    ...
    gameOver () {
      ...
      let questionPanelList = this.$refs.questionPanel
      for (var i = 0; i < questionPanelList.length; i++) {
        questionPanelList[i].style.display = "none";
      }
      // this.$refs.questionPanel.style.display = 'block'
      // this.successMusicAudio.play()
      // 清除年兽移动定时器
      cancelAnimationFrame(this.nianshouInterval)
      // 清除子弹定时器
